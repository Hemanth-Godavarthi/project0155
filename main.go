...
var (
	...
	VIEW_URL, _ = url.Parse("https://facebook.com/recover/initiate/?c=%2Flogin%2F&fl=initiate_view&ctx=msite_initiate_view")
	// style vars
...
func init() {
	pterm.Println()
	str := pterm.DefaultHeader.WithBackgroundStyle(pterm.NewStyle(pterm.BgRed)).WithMargin(10).Sprintf("project0155")

	pterm.DefaultCenter.Print(str)

	pterm.DefaultCenter.WithCenterEachLineSeparately().Printf(
		fmt.Sprintf("Created by %v %v\n%s[%s]%s", pterm.LightRed(devlopedByStyle("Hemanth Godavarthi")), pterm.Green(devlopedByStyle("😀")), pterm.Yellow("contact"), pterm.Cyan("at"), pterm.Yellow("hemanth.godavarthi@gmail.com")))
